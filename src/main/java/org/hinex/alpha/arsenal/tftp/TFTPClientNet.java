package org.hinex.alpha.arsenal.tftp;
...
/**
 * TFTP Client Using Java IO
 * @author http://javapapers.com/java/java-tftp-client/
 */
public class TFTPClientNet {
    
    /*
     * TFTP Protocol As per RFC 1350 
     * opcode - operation 
     * 1 - Read request (RRQ) 
     * 2 - Write request (WRQ) 
     * 3 - Data (DATA) 
    ...
    private void get(String fileName) throws IOException {
        ...
        outBoundDatagramPacket = new DatagramPacket(requestByteArray, requestByteArray.length, inetAddress, TFTP_DEFAULT_PORT);
    ...
    private ByteArrayOutputStream receiveFile() throws IOException {
        ...
        do {
            ...
            inBoundDatagramPacket = new DatagramPacket(bufferByteArray, bufferByteArray.length, inetAddress, datagramSocket.getLocalPort());
